...
function fitSize(w,h){
  const max=1536,scale=Math.min(1,max/Math.max(w,h));
  return {w:Math.max(320,Math.round((w*scale)/16)*16),h:Math.max(320,Math.round((h*scale)/16)*16)};
}
...
export default function Studio(){
  ...
  async function signed(bucket,path){
    if(!path)return "";
    const {data,error}=await supabase.storage.from(bucket).createSignedUrl(path,3600);if(error)throw error;return data?.signedUrl||"";
  }
  function redraw(){
    const d=displayRef.current,s=sourceRef.current,m=maskRef.current;if(!d||!s||!m)return;
    const ctx=d.getContext("2d");ctx.clearRect(0,0,d.width,d.height);ctx.drawImage(s,0,0);ctx.save();ctx.globalAlpha=.38;ctx.drawImage(m,0,0);ctx.restore();
  }
  async function initialize(url){
    const img=await loadImage(url),size=fitSize(img.naturalWidth,img.naturalHeight);
  ...
  function paintLine(a,b){
    const m=maskRef.current,ctx=m.getContext("2d");ctx.save();ctx.lineCap="round";ctx.lineJoin="round";ctx.lineWidth=brush*(m.width/900);
    if(mode==="erase"){ctx.globalCompositeOperation="destination-out";ctx.strokeStyle="#000";}else{ctx.globalCompositeOperation="source-over";ctx.strokeStyle="#fff";setHasMask(true);}
  ...
  async function buildOpenAIMask(){
    const select=maskRef.current,c=document.createElement("canvas");c.width=select.width;c.height=select.height;const x=c.getContext("2d");x.fillStyle="#fff";x.fillRect(0,0,c.width,c.height);x.globalCompositeOperation="destination-out";x.drawImage(select,0,0);x.globalCompositeOperation="source-over";return c;
  }
  ...
  async function generate(){
    ...
    try{
      ...
      const ai=await loadImage(data.image),final=document.createElement("canvas");final.width=sourceRef.current.width;final.height=sourceRef.current.height;const f=final.getContext("2d");f.drawImage(sourceRef.current,0,0);
      ...
      const finalBlob=await canvasBlob(final),stamp=Date.now(),base=`${sim.organization_id}`,resultPath=`${base}/results/${sim.id}-${stamp}.png`,maskPath=`${base}/masks/${sim.id}-${stamp}.png`;
      const [upResult,upMask]=await Promise.all([supabase.storage.from("simulation-results").upload(resultPath,finalBlob,{contentType:"image/png",upsert:false}),supabase.storage.from("simulation-results").upload(maskPath,maskBlob,{contentType:"image/png",upsert:false})]);
      ...
      setResultUrl(URL.createObjectURL(finalBlob));setSim({...sim,status:"ready",result_image_path:resultPath,mask_path:maskPath});setMsg("Simulação pronta. Fora da área pintada, a fotografia original foi preservada pixel a pixel.");
    }catch(e){setMsg(e?.message||"Não foi possível gerar a simulação.");if(sim)await supabase.from("simulations").update({status:"failed",error_message:e?.message||"Falha na geração"}).eq("id",sim.id);}finally{setBusy(false);}
  }
  ...
  return <main className="studioPage">
    ...
    <section className="studioGrid"><div className="studioCanvasWrap"><canvas ref={displayRef} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp}/><canvas ref={sourceRef} style={{display:"none"}}/><canvas ref={maskRef} style={{display:"none"}}/></div>
      <div className="panel studioTools" style={{marginTop:0}}><h2>1. Marque a superfície</h2><p className="muted">Passe o dedo ou mouse somente sobre a parte visível que deve receber o material. Não pinte móveis, portas ou objetos.</p><div className="toolToggle"><button className={mode==="paint"?"primary":"secondary"} onClick={()=>setMode("paint")}>🖌️ Pintar área</button><button className={mode==="erase"?"primary":"secondary"} onClick={()=>setMode("erase")}>⌫ Apagar</button></div><div className="field"><label>Tamanho do pincel</label><input type="range" min="12" max="110" value={brush} onChange={e=>setBrush(Number(e.target.value))}/></div><button className="secondary" onClick={clearMask}>Limpar seleção</button>
        <div className="materialCard">{textureUrl&&<img src={textureUrl} alt={product.name}/>}<div><b>{product.name}</b><div className="muted">{product.brand||""}{product.piece_width_cm&&product.piece_height_cm?` • ${product.piece_width_cm}×${product.piece_height_cm} cm`:""}</div></div></div><button className="primary generateButton" disabled={busy||!hasMask} onClick={generate}>{busy?"Gerando...":"✨ Gerar prévia com IA"}</button><div className="muted" style={{fontSize:12}}>A IA usa o material cadastrado como referência. O PREVIEW recompõe o resultado sobre a foto original e mantém intactos todos os pixels fora da sua seleção.</div>
      </div></section>
  </main>;
}
